defmodule MCP.MessageRouterTest do
  ...
  defmodule MockMCPServer do
    ...
    @impl true
    def handle_ping(_assigns, request_id) do
      {:ok, %{jsonrpc: "2.0", id: request_id, result: %{}}}
    end

    @impl true
    def handle_initialize(_assigns, request_id, params) do
      capabilities = params["capabilities"]
      {:ok, %{jsonrpc: "2.0", id: request_id, result: %{"capabilities" => capabilities}}}
    end

    @impl true
    def handle_complete(_assigns, request_id, _params) do
      {:ok, %{jsonrpc: "2.0", id: request_id, result: %{completion: %{}}}}
    end

    @impl true
    def handle_list_prompts(_assigns, request_id, _params) do
      {:ok, %{jsonrpc: "2.0", id: request_id, result: %{prompts: []}}}
    end

    @impl true
    def handle_get_prompt(_assigns, request_id, params) do
      name = params["name"]
      {:ok, %{jsonrpc: "2.0", id: request_id, result: %{"description" => name}}}
    end

    @impl true
    def handle_list_resources(_assigns, request_id, _params) do
      {:ok, %{jsonrpc: "2.0", id: request_id, result: %{resources: []}}}
    end

    @impl true
    def handle_read_resource(_assigns, request_id, _params) do
      {:ok, %{jsonrpc: "2.0", id: request_id, result: %{contents: []}}}
    end

    @impl true
    def handle_list_tools(_assigns, request_id, _params) do
      {:ok, %{jsonrpc: "2.0", id: request_id, result: %{tools: []}}}
    end

    @impl true
    def handle_call_tool(_assigns, request_id, _params) do
